    ...
    functionRunLambdaNodeJs(evt) {
      ...
      return new BbPromise(function(resolve) {
        ...
        try {
          ...
          let functionHandler = require(functionFile);
          let handlerProp = _this.function.handler.split('/').pop().split('.')[1];
          functionHandler = functionHandler[handlerProp] || functionHandler.default || functionHandler;

          // Fire function
          let functionEvent = _this.evt.options.eventJson || SUtils.readAndParseJsonSync(path.join(_this.function._config.fullPath, 'event.json'));
